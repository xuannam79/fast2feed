        ...
        return DB::table('orders')->join('customer',function($join)
            {
                $join->on('customer.customer_id','=','orders.customer_id')
                ->join('account','account.account_id','=','customer.account_id');
            })->select('account.account_id','orders.status_2','customer.customer_name','orders.order_id','orders.date_create','orders.total','orders.payment')->where('account.account_id', $accId)->get();
    }
